...
describe('ProxyService — orchestration', () => {
  ...
  describe('explicit model routing', () => {
    ...
    it('applies x-manifest-reasoning-effort to canonical direct model ids', async () => {
      ...
      // Explicit model ids bypass the tier/model-params merge; the header is
      // still applied to the provider body and the request metadata remains
      // null, matching upstream's direct-route snapshot contract.
      expect(result.meta.request_params).toBeNull();
    });
